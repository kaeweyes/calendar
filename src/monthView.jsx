// ===== monthView.jsx (updated to show only first event per day cell and make day popup scrollable)
// plus larger nav, larger date numbers, and maroon "No events" + date font
...
const toISODate = (d) => {
  // produce local YYYY-MM-DD (avoid UTC shift from toISOString)
...
// -------------------- ADDED HELPERS (for dd/mm and default times) --------------------
// format dd/mm/yyyy
...
// get default start/end rounded to next 30-min slot (strictly after now)
function getDefaultStartEnd() {
  const now = new Date();
  const mins = now.getHours() * 60 + now.getMinutes();
  // strictly after now -> add 1 then ceil to 30
...
/* Convert older repeat formats to new unified format for compatibility */
function normalizeRepeat(series) {
  // new format: { repeatWeeks: ['week1',..], repeatFreq: 'daily'|'monthly'|'quarterly'|'none' }
  if (series.repeatWeeks || series.repeatFreq) return series; // already new
  ...
  // if repeat was an array or string like before
...
function occurrencesForSeriesInRange(seriesRaw, monthStart, monthEnd) {
  ...
  // if single event and before month range
  ...
  for (let d = new Date(monthStart); d <= monthEnd; d.setDate(d.getDate() + 1)) {
    ...
    // months difference from start (can be negative if before start)
    const monthsDiff = (d.getFullYear() - start.getFullYear()) * 12 + (d.getMonth() - start.getMonth());

    // If user selected week-based options, use week-of-month + weekday logic and apply frequency as a month-step filter
    if (repeatWeeks.length) {
      // determine if this month is allowed by the chosen frequency
      let freqOk = false;
      if (repeatFreq === 'daily') {
        freqOk = monthsDiff >= 0;
      ...
      // date-based occurrences (no week check)
      if (repeatFreq === 'none') {
    ...
    // always include the original start date if it falls in range (helps one-off events)
...
const styles = {
  ...
  // larger date number in the top-right of each cell; serif font and maroon color
...
const Table = ({ year, month, seriesArray, onServerRefresh, onCreateFromDay }) => {
  ...
  const [editOcc, setEditOcc] = useState(null); // { occ, fromDate }
  ...
  // Move a single occurrence (create a one-off event on target date and exclude original)
  async function handleMoveOccurrence(seriesId, fromDateStr, toDateStr, occ) {
    if (fromDateStr === toDateStr) return;
    try {
      // add exclusion to original series so it no longer occurs on the old date
      await addExclusionOnServer(seriesId, fromDateStr);

      // create a new single-date series representing the moved occurrence
      const newSeries = {
        id: `ts-${Date.now()}`,
        name: occ.name,
        startDate: toDateStr,
  ...
  // Edit a single occurrence — if original was a single-event series, replace it; otherwise exclude original date and create a new single event
  async function handleEditOccurrence(seriesId, originalDate, updated) {
    // updated: { name, date: newDate, startTime, endTime }
    try {
      // Strategy: create new single series with updated data, then either delete original series (if single) or add exclusion to original series for originalDate
      const newSeries = {
        id: `ts-${Date.now()}`,
        name: updated.name,
      ...
      // Now mark original occurrence as excluded OR delete original if single
      if (updated._originalWasSingle) {
        // if it was a single series, delete it
        await deleteSeriesOnServer(seriesId);
      } else {
        // else add exclusion for that date
    ...
    <div>
          ...
          {calendarGrid.map((row, rI) => (
            <tr key={rI}>
              {row.map((cell) => {
                ...
                // only preview the first event (if any)
      ...
      {openCell && (
        <Popup onClose={() => setOpenCell(null)} title={`Events — ${openCell.dateStr}`}>
          <div>
            {/* make the events list scrollable so long lists are usable */}
      ...
      {editOcc && (
        <EditEventPopup
          ...
          onSave={async (updated) => {
            // determine if original was single-event (no repeats)
...
// EditEventPopup: edit a single occurrence (creates a new one-off and excludes or removes original as appropriate)
...
// CreateEventPanel — MODIFIED per your request (name suggestions, dd/mm/yyyy, next 30-min defaults, freq-first, conditional weeks)
const CreateEventPanel = ({ defaultDate, onCreate, onCancel, recentNames = [] }) => {
  const [name, setName] = useState("");

  // normalize incoming defaultDate (ISO or dd/mm) -> dd/mm for text input
  ...
  const [date, setDate] = useState(defaultDD); // dd/mm/yyyy string
  const [startTime, setStartTime] = useState(defaultTimes.start);
  const [endTime, setEndTime] = useState(defaultTimes.end);
  const [repeatWeeks, setRepeatWeeks] = useState([]);
  const [repeatFreq, setRepeatFreq] = useState('none'); // default: no repeats
  ...
  return (
    <div style={styles.sidePanel}>
      ...
      {/* wrap form so Enter on any input triggers create */}
...
export default function App() {
  ...
  // read year/month from query params (so /month?year=2025&month=3 opens March 2025)
  ...
  // --- recent names used for datalist suggestions (most recent first, deduped)
  ...
  return (
    <div style={styles.app}>
      ...
      <div style={{ marginTop: 12 }}>
        ...
        {/* Back to Dashboard moved below the table as requested */}
        <div style={{ marginTop: 12 }}>
