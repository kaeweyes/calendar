...
const getDefaultStartEnd = () => {
  ...
  // round up to next 30-minute slot (strictly after now)
...
/* occurrences (ensure startD is defined) */
...
/* styles — month cells have limited height so inner list can scroll */
const styles = {
  // Force app to viewport height and prevent page scroll — internal panels will scroll
  app: {
    ...
    overflow: "hidden" // critical: prevents document scroll
  },
  header: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 },
  // grid forced into 4 equal rows so it never needs to scroll as a whole
  yearGrid: {
    display: "grid",
    gridTemplateColumns: "repeat(3, 1fr)",
    gridTemplateRows: "repeat(4, 1fr)", // 4 rows, equal heights
    gap: 12,
    height: "100%",   // fill the left column height
    minHeight: 0,
  },
  monthBlock: {
  ...
  // make the event count larger and maroon
  meta: { fontSize: 14, color: "maroon", fontWeight: 700 },
  // *** IMPORTANT FIX: make this a grid that can grow inside a flex child and then scroll
  eventsList: {
    marginTop: 8,
    display: "grid",
    gridTemplateColumns: "1fr 1fr", // NX2 layout
    gap: 8,
    overflowY: "auto", // only the cell contents scroll
    flex: 1,            // allow this to take remaining space inside the month block
    minHeight: 0,
    boxSizing: "border-box",
    paddingRight: 6,
    gridAutoRows: "min-content", // ensure rows size to content, not stretch
    alignContent: "start",
  },
  ...
  // No events now maroon and larger
...
// ConfirmPopup: simple two-button confirmation modal
...
const SchedulePopup = ({ unscheduled, year, monthIndex, onCancel, onScheduled, recentNames = [] }) => {
  ...
  // keep ref updated to latest submit implementation
  useEffect(() => {
    submitRef.current = submit;
  }, [submit, name, date, startTime, endTime, repeatWeeks, repeatFreq]);

  // global Enter handler while popup is open -> schedule
  useEffect(() => {
    const onKey = (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
  ...
  return (
    <Popup onClose={onCancel} title={`Schedule "${unscheduled?.name || ''}" — ${["JAN","FEB","MAR","APR","MAY","JUNE","JUL","AUG","SEPT","OCT","NOV","DEC"][monthIndex]} ${year}`}>
      <form onSubmit={(e) => { e.preventDefault(); submit(); }} style={{ display: 'grid', gap: 8 }}>
        ...
        {/* show week-picker only when monthly or quarterly */}
...
/* Main Home component — month cells show all events and inner list scrolls */
export default function Home({ yearOverride } = {}) {
  ...
  // Confirmation state for deleting full series (triggered from unscheduled list)
  ...
  /* compute recent names for datalist suggestions (most recent by createdAt then unscheduled) */
  ...
  return (
    <div style={styles.app}>
      ...
      {/* main area: use flex and minHeight:0 so inner columns can size properly */}
      <div style={{ marginTop: 12, display: 'flex', gap: 12, flex: 1, minHeight: 0, overflow: "hidden" }}>
        {/* left months column — no overflow on this container */}
              ...
              {months.map((mName, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* wrapper allows inner list to shrink and provides clipping */}
                    <div style={{ marginTop: 8, flex: 1, minHeight: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                      {occs.length === 0 ? (
                        ...
                        // the inner list is scrollable — two-column grid now
            ...
            {unscheduled.length === 0 ? (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {/* <button style={styles.smallBtn} onClick={async () => { await navigator.clipboard?.writeText(u.name).catch(()=>{}); }}>Copy</button> */}
                  <button
      ...
      {/* Confirmation popup for deleting unscheduled item + its series */}
